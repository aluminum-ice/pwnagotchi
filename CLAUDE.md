...
| Install Python pkg + system files (requires root for the system-file copy) | `sudo python3 setup.py install` — requires `requirements.txt` to exist; on the image this is generated by pip-compile in the Ansible build; locally you must create it first |
...
- `requirements.txt` is **not committed to the repo** — it is a build artifact generated by pip-compile during the image build (Ansible task: "Regenerate requirements.txt from requirements.in"). Edit `requirements.in` instead. `requirements.txt` is in `.gitignore`. Do not add it back.
- `requirements.in` contains the version constraints for pip-compile. All key packages are pinned (see inline comments). The pins are intentionally conservative to match what builds on armhf Python 3.9 via piwheels. Notably: `gym==0.14.0` (not 0.19.x — broken metadata in pip>=24.1), `flask~=1.0` (Flask 3.x is Phase 2), `websockets~=8.1` (bettercap.py API), `numpy~=1.21.4` (stable-baselines 2.x compat).
- `pwnagotchi.json` (Packer template) uploads `requirements.in` to the image as a separate file provisioner so pip-compile can find it. The sdist does not include `requirements.in`.
...
- `.mo` locale files are compiled build artifacts — do not commit them. Run `make langs` to regenerate. They are in `.gitignore`.
...
- [ ] pip-compile runs cleanly during image build on Python 3.11 Bookworm armhf; `pycryptodome` absent from generated `requirements.txt`
...
- [ ] `requirements-aarch64.txt` committed and generated on aarch64 Python 3.11 (this file IS committed to the repo as an explicit aarch64 lock file, unlike the armhf `requirements.txt` which is a build artifact)
