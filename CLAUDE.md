...
|`Automata`    |`pwnagotchi/automata.py`                                                           |Mood state machine: tracks `_epoch` (`ai/epoch.py`) and fires `set_bored`/`set_sad`/`set_excited`/`set_lonely`/`set_angry`. Note: `set_grateful`/`in_good_mood`/`_has_support_network_for` are dead code pending removal in Phase 1.5.|
...
1. Delete `on_unread_messages`, `on_new_peer`, `on_lost_peer` from `voice.py`
1. Run `scripts/language.sh update <lang>` for every language under `pwnagotchi/locale/`
1. Run `make langs`
1. Commit: `Phase 1.5: remove orphaned peer/inbox voice methods and stale i18n strings`
...
1. In each file, remove only the `friend_face` and `friend_name` keys from the dict returned by `layout()`. Touch nothing else.
1. Verify: `grep -r "friend_face\|friend_name" pwnagotchi/ui/hw/` returns nothing
1. Commit: `Phase 1.5: remove dead friend_face/friend_name layout entries from ui/hw modules`
...
**Files:** `pwnagotchi/automata.py`, `pwnagotchi/defaults.toml`

**Context:** `_has_support_network_for()` now returns `False` unconditionally (peers are gone). This makes `in_good_mood()` always `False`, making every `else: self.set_grateful()` branch in the mood setters unreachable dead code.

**Steps:**

1. Run scope grep first — review before touching anything:
   `grep -n "set_grateful\|in_good_mood\|_has_support_network_for\|bond_encounters_factor" pwnagotchi/automata.py pwnagotchi/defaults.toml pwnagotchi/voice.py`
1. Delete `_has_support_network_for`, `in_good_mood`, and `set_grateful` from `automata.py`
1. In `set_lonely`, `set_bored`, `set_sad`, `set_angry`, and `next_epoch`: remove the `else: self.set_grateful()` branch and any surrounding `if self.in_good_mood():` condition. The mood-setting logic itself is unchanged.
1. If `bond_encounters_factor` has no remaining references, remove it from `defaults.toml`
1. Run `python -m py_compile pwnagotchi/automata.py`
1. Commit: `Phase 1.5: remove unreachable grateful mood logic from automata.py`

**Prompt for Claude Code:**

> *“Read `docs/DESIGN.md` Section 4.3. Step 1: run `grep -n 'set_grateful\|in_good_mood\|_has_support_network_for\|bond_encounters_factor' pwnagotchi/automata.py pwnagotchi/defaults.toml pwnagotchi/voice.py` and show me the output so we can confirm the full scope before touching anything. Step 2: delete `_has_support_network_for`, `in_good_mood`, and `set_grateful` from `automata.py` — show me the diff. Step 3: in `set_lonely`, `set_bored`, `set_sad`, `set_angry`, and `next_epoch`, remove the `else: self.set_grateful()` branch and any `if self.in_good_mood():` condition wrapping it — show me each diff before applying. Step 4: if `bond_encounters_factor` has no remaining references, remove it from `defaults.toml`. Step 5: run `python -m py_compile pwnagotchi/automata.py`. Commit.”*

**Acceptance:** `grep -r "set_grateful\|in_good_mood\|_has_support_network_for" pwnagotchi/` returns nothing. `python -m py_compile pwnagotchi/automata.py` exits 0.
...
1. Run: `python -m pytest tests/test_phase1_removal.py -v` — all 11 must pass
1. Commit: `Phase 1.5: add regression guards for dead code sweep`
...
- [ ] `grep -r "bond_encounters_factor" pwnagotchi/` returns nothing (if removed)
- [ ] `python -m py_compile pwnagotchi/automata.py` exits 0
...
1. `test_no_grid_keys_in_defaults` — assert `main.plugins.grid` does not appear in `defaults.toml` *(Phase 1 regression guard)*
