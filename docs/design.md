...
2. `test_identity_module_not_importable` — `import pwnagotchi.identity` raises `ModuleNotFoundError`
...
2. Run `scripts/language.sh update <lang>` for each language directory to regenerate `.pot` and merge into each `.po`, which will mark the removed strings as obsolete
3. Run `make langs` to recompile all `.mo` files
4. Commit voice.py change and i18n update as a single commit
...
2. In each file, remove the `friend_face` and `friend_name` keys from the layout dict returned by `layout()`. Remove only those two keys; do not touch any other layout entry.
3. Do not change the method signature, the class structure, or any other key.
4. Commit as a single commit covering all `ui/hw/` files.
...
**What:** After Phase 1, `_has_support_network_for()` returns `False` unconditionally because `self._peers` no longer exists. This makes `in_good_mood()` always return `False`. The `set_grateful()` call sites in `automata.py` are therefore unreachable. However, `in_good_mood()` is **not** purely internal to `automata.py` — it has a live external caller in `pwnagotchi/ui/view.py:209`:

```python
good_mood = self._agent.in_good_mood()
```

This call executes on every wait/look-around cycle in `View.wait()`. The return value is always `False` (because `_has_support_network_for()` always returns `False`), so the call is **unreachable-valued but not unreachable** — deleting `in_good_mood()` without first patching `view.py` introduces a runtime `AttributeError` in the UI render loop.

The affected dead-code sites in `automata.py` are the `else: self.set_grateful()` branches in:
- `set_lonely()`
- `set_bored()`
- `set_sad()`
- `set_angry()`
- `next_epoch()` (an `elif` block at lines 132–133)

Additionally, once the `if not self._has_support_network_for(...)` conditions are removed from `set_bored()` and `set_sad()`, the local variable `factor` that was computed only to feed `_has_support_network_for()` becomes an unused local and should be removed in the same sweep. In `set_angry()`, `factor` is a method parameter — the signature must stay unchanged.

**Files:** `pwnagotchi/automata.py`, `pwnagotchi/ui/view.py`

> **Note on `bond_encounters_factor`:** `pwnagotchi/ai/epoch.py:87` reads `bond_encounters_factor` unconditionally from config (`bond_unit_scale = self.config['personality']['bond_encounters_factor']`). It is **not** only referenced by `_has_support_network_for()`. It must remain in `defaults.toml`. Removing it is a Phase 3 concern when `ai/epoch.py` is rewritten. Do not remove it in this phase.

**What to do:**

1. Run the scope grep and review the output before touching anything:
...
2. In `pwnagotchi/ui/view.py`, replace `good_mood = self._agent.in_good_mood()` with `good_mood = False`. This is behaviour-preserving: `in_good_mood()` already always returns `False`, so the UI already always selects the non-happy face variants.
3. Delete `_has_support_network_for()`, `in_good_mood()`, and `set_grateful()` entirely from `automata.py`
4. In `set_lonely()`, `set_bored()`, `set_sad()`, `set_angry()`, and `next_epoch()`: remove the `else: self.set_grateful()` branch and the `if not self._has_support_network_for(...):` condition wrapping it. Dedent the remaining mood-setting body. The mood-setting logic itself is unchanged.
5. In `set_bored()` and `set_sad()`, remove the now-unused local `factor` variable (it was computed solely to pass to `_has_support_network_for()`). Do not touch the `factor` parameter in `set_angry()`.
6. `bond_encounters_factor` remains in `defaults.toml` — do not remove it.
...
>
> *Step 2: In `pwnagotchi/ui/view.py`, find the line `good_mood = self._agent.in_good_mood()` and replace it with `good_mood = False`. Show me the diff before applying. Do not touch anything else in view.py.*
>
> *Step 3: Delete `_has_support_network_for`, `in_good_mood`, and `set_grateful` from `pwnagotchi/automata.py`. Show me the diff.*
>
> *Step 4: In `set_lonely`, `set_bored`, `set_sad`, `set_angry`, and `next_epoch`, remove the `else: self.set_grateful()` branch and the `if not self._has_support_network_for(...):` condition. Dedent the remaining mood-setting body so it runs unconditionally. Show me each method diff before applying.*
>
> *Step 5: In `set_bored` and `set_sad`, remove the now-unused local `factor` variable that was only computed to feed `_has_support_network_for`. Do not touch the `factor` parameter in `set_angry`. Show me the diff.*
>
> *Step 6: Do NOT remove `bond_encounters_factor` from `defaults.toml` — it is consumed by `pwnagotchi/ai/epoch.py:87` and must remain.*
>
> *Step 7: Run `python -m py_compile pwnagotchi/automata.py pwnagotchi/ui/view.py` and confirm both exit 0. Commit."*

**Acceptance:**
- `grep -r "set_grateful\|in_good_mood\|_has_support_network_for" pwnagotchi/` returns nothing
- `grep "good_mood = self._agent.in_good_mood" pwnagotchi/ui/view.py` returns nothing (replaced with `good_mood = False`)
- `grep "bond_encounters_factor" pwnagotchi/defaults.toml` still returns the entry (it was not removed)
- `grep "bond_encounters_factor" pwnagotchi/ai/epoch.py` still returns the entry (AI stack untouched)
- `python -m py_compile pwnagotchi/automata.py pwnagotchi/ui/view.py` exits 0
- The remaining mood methods (`set_lonely`, `set_bored`, `set_sad`, `set_angry`) still exist and are callable
...
| Remove grateful mood logic | `pwnagotchi/automata.py`, `pwnagotchi/ui/view.py` | grep for set_grateful/in_good_mood/\_has_support_network_for returns nothing; view.py uses `good_mood = False` |
| `bond_encounters_factor` retained | `pwnagotchi/defaults.toml` (no change) | Entry still present — consumed by ai/epoch.py; removal is Phase 3 |
| Regression tests | `tests/test_phase1_removal.py` | 11 tests pass |
| CHANGELOG.md entry | `CHANGELOG.md` | Phase 1.5 section documents all removals and the defect correction |

### 4.6 Phase 1.5 Sequence

1. **Sub-task A:** Remove orphaned `voice.py` methods → update i18n → commit
2. **Sub-task B:** Remove dead `friend_face`/`friend_name` layout keys from all `ui/hw/` modules → commit
...
4. **Sub-task D:** Add 3 regression tests to `tests/test_phase1_removal.py` → confirm 11 tests pass → commit
5. Update `CHANGELOG.md` → commit
6. Tag **v1.9.1**
...
2. Set a module-level `AI_AVAILABLE = False` flag in `pwnagotchi/ai/__init__.py`
3. In `agent.py`, check `AI_AVAILABLE` before entering AI mode; fall back to `MANU` cleanly
4. Add a small UI indicator so the operator knows AI is inactive
...
5. `test_step_obs_within_bounds` — `observation_space.contains(obs)` is `True` after step with random valid action
...
2. Migrate `setup.py` to `pyproject.toml`.
3. Upgrade Flask stack (3.x); remove MarkupSafe cap and fix itsdangerous comment.
4. Apply Python 3.7→3.11 compat fixes.
5. Re-run pip-compile (Python 3.11, Bookworm). Commit `requirements.txt`.
6. Harden AI fallback guard.
7. Add ruff + pre-commit. Write `tests/test_config.py`, `test_bettercap.py`, `test_plugins.py`, `test_ai_guard.py` (17 tests total). All must pass.
8. Migrate USB networking to NetworkManager profile.
9. Tag **v2.0.0-beta**. Community test (2–4 weeks). Tag **v2.0.0-stable**.
...
2. Update bettercap Go build task: `GOARCH=arm64`.
3. Boot test image on RPi Zero 2 W 64-bit. Verify bettercap, nexmon, display, plugins.
4. Rewrite gymnasium env adapter. Write `tests/test_gym_env.py` (7 tests). Run env_checker. All must pass.
5. Integrate SB3 + PyTorch. Write `tests/test_sb3_integration.py` (3 slow tests). Run on aarch64.
6. Update `requirements.in`. Run pip-compile (aarch64). Commit `requirements-aarch64.txt`.
7. Add QEMU arm64 CI job.
8. Tag **v3.0.0-beta**. Community test. Tag **v3.0.0-stable**.
...
| 4 | `set_grateful`, `in_good_mood`, `_has_support_network_for` removed from `automata.py` | `grep -r "set_grateful\|in_good_mood\|_has_support_network_for" pwnagotchi/` returns nothing |
| 5 | `view.py` `in_good_mood()` call replaced with `good_mood = False` | `grep "in_good_mood" pwnagotchi/ui/view.py` returns nothing |
| 6 | `bond_encounters_factor` retained in `defaults.toml` (consumed by `ai/epoch.py`) | `grep "bond_encounters_factor" pwnagotchi/defaults.toml` returns entry; `grep "bond_encounters_factor" pwnagotchi/ai/epoch.py` returns entry |
| 7 | All 11 tests pass | `python -m pytest tests/test_phase1_removal.py -v` (8 original + 3 new guards) |
| 8 | `python -m py_compile pwnagotchi/automata.py pwnagotchi/ui/view.py` exits 0 | No syntax errors |
