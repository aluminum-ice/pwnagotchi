...
# Read install_requires from requirements.in rather than requirements.txt.
#
# requirements.txt is a build artifact generated by pip-compile inside the
# image build environment (armhf Python 3.9, piwheels). It is intentionally
# not committed to the repo and will not exist on a developer workstation.
#
# requirements.in is the source of truth for dependency constraints. We parse
# it directly here, skipping:
#   - blank lines
#   - comment lines (start with #)
#   - pip option lines (start with -- or -)
#   - index URL lines (start with --extra-index-url or --index-url)
#
# The resulting list is passed to install_requires. During the image build,
# pip-compile has already resolved and locked the full dependency tree into
# requirements.txt, so pip install uses that locked file (via --ignore-installed)
# rather than re-resolving from the constraints here. The install_requires list
# is used by pip to communicate dependencies in the sdist metadata; it does not
# override the locked resolution during the image build.
